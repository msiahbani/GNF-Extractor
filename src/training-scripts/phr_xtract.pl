...
if (defined $cfg->{TRAIN_DIR}) {
    ...
    print STDERR "\nRunning Moses training steps on TRAIN set ...haha file name $step_lc_dir and $phr_align_dir\n";
...
sub run_preprocess {
    ...
    for my $pre ( split(/,/, $cfg->{$step_pre}) ) {
        $step_dir = "${step}_DIR";
        $step_lc_dir = "LC_${step}_DIR";
	print "Here damn it $step_dir\n";
	print "Haha same here $step_lc_dir\n";
        ...
        $cmd = "cat $cfg->{$step_dir}/$src_pre.$cfg->{src} >> $cfg->{$step_lc_dir}/$comb_pre.$cfg->{src}";
        safeSystem($cmd);
        $cmd = "cat $cfg->{$step_dir}/$tgt_pre.$cfg->{tgt} >> $cfg->{$step_lc_dir}/$comb_pre.$cfg->{tgt}";
        safeSystem($cmd);
    }
}
...
sub pp_clean_corpus {
    ...
    print "    ** Clearning corpus turned on ...\n";
...
sub pp_lc_nd_tok {
    ...
    if ($cfg->{$what_pp} =~ /lc/) {
        ...
        $cmd = "perl $pl_scrpt < $cfg->{$step_dir}/$new_pre.$cfg->{$src_or_tgt} > $cfg->{$step_lc_dir}/$pre.lc.$cfg->{$src_or_tgt}";
	print "---------------------------------\n-------------------\n---------------\n";
	print $cmd ,"\n";
        safeSystem($cmd);
        $new_pre = "$pre.lc";
    ...
    if ($cfg->{$what_pp} =~ /tok/) {
        ...
        $cmd = "perl $pl_scrpt -l $cfg->{$src_or_tgt} < $cfg->{$step_dir}/$new_pre.$cfg->{$src_or_tgt} > $cfg->{$step_lc_dir}/$new_pre.tok.$cfg->{$src_or_tgt}";
        safeSystem($cmd);
        $new_pre = "$new_pre.tok";
    }
